...
| `ldap` | **Functional** | |
| `oauth` | **Functional** | |
| `jwt` | **[FIXED before this pass]** `hmac_secret` exists; an HMAC algorithm without a secret is rejected at config validation. Claim is stale | RSA/EC verification works; HS256/384/512 accepted in alg list + verify switch but no symmetric-key source, so HMAC always fails with "invalid key type for HMAC" (`jwt.go:462-468,191-201,628-645`). Not exploitable (alg allowlist blocks RS↔HS confusion). |
...
| `mfa_wrapper` | **[FIXED]** Metadata already matched (inline) before this pass; the real gap was `ValidateConfig` accepting the by-name format `Create()` rejects — both now refuse it identically, and it is now in the UI provider list | Works only with inline `primary`/`secondary` blocks; `DefaultConfig`/`ConfigSchema` advertise a by-name format that `Create()` hard-errors (`internal/auth/mfa/plugin.go:45-48,80-88,91-139`). Correct format IS documented externally (authentication.mdx:429-459), so "unusable without source" is *overstated*. Not in UI. |
| `ntlm` | **[FIXED]** Reports `unimplemented`; `ValidateConfig` now refuses every config and the factory refuses the provider type, so it cannot be selected in the UI or saved. Note this makes an `auth.negotiate` block with an `ntlm_provider` a startup failure | `handleType3` and `ValidateAuthenticate` unconditionally return `ErrVerificationUnsupported`; rejects 100% of clients (`internal/auth/plugin/ntlm/ntlm.go:200-203,268-281,566`). Still selectable in UI with full form and no warning. |
| `system` (PAM) | **[FIXED]** Reports `build_disabled` when compiled without PAM, surfaced at startup and as a UI badge via `GET /api/v1/auth/plugins`. Deliberately *not* refused — the same config is correct on Windows, macOS and a `-tags pam` build | `pam_stub.go:40-45` `validateLinux` logs a warn and returns false unless built with `linux && cgo && pam`. Docker (`CGO_ENABLED=0`, no `-tags pam`) and `make build` both omit it. Offered as "System (PAM)" with a PAM Service field, no UI warning. |
| `negotiate` | **[FIXED]** Still (correctly) not a plugin, but `type: negotiate` is now refused with an error pointing at `auth.negotiate.*`, and that refusal is reachable from the UI's save/validate endpoints | Not registered via `RegisterPlugin`; real SPNEGO/NTLM handshake lives in `internal/server/negotiate.go` driven by `auth.negotiate.*`, which has no UI. Docs `mode: negotiate` examples are doubly broken (see §9). |
