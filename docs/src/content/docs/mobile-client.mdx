...
> **Note:** the mockup above is aspirational for the Connection Details and
> Network sections: the REST API does **not** currently report protocol,
> encryption, port, MTU, local IP, gateway, DNS servers or interface name, so
> the app shows only the fields the API supplies (traffic counters, session
> duration, connection counts, DNS statistics). Anything shown describes the
> **remote Bifrost client** the app controls, never a tunnel on the phone —
...
│  ─────────────────────────────────  │
│  Auto-connect              [  🔘  ] │
│  Remote client: connect on start    │
│                                     │
│  Split Tunneling                    │
