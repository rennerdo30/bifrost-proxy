...
|--------|------|---------|-------------|
| `system_proxy.enabled` | bool | `false` | Enable automatic OS proxy configuration |
| `proxy.http.listen` | string | — | HTTP listener address; the listener is active when this is set |
| `proxy.socks5.listen` | string | — | SOCKS5 listener address; the listener is active when this is set |

:::note[No per-listener `enabled` flag]
Listeners have no `enabled` field. A listener runs when its `listen` address is
set and is skipped when the address is empty or omitted.
:::
...
When both HTTP and SOCKS5 listeners have a `listen` address, the system proxy uses this priority:

1. **HTTP listener** (`proxy.http.listen`) - Most compatible with system applications
2. **SOCKS5 listener** (`proxy.socks5.listen`) - Used only when `proxy.http.listen` is empty
...
**Cause**: System proxy is enabled but neither HTTP nor SOCKS5 listener is running — that is, neither has a `listen` address.

**Solution**: Give at least one proxy listener an address:
```yaml
proxy:
  http:
    listen: "127.0.0.1:8080"
...
- **Bypass list on macOS and Linux**: Windows already gets a `<local>` bypass
  written automatically when the profile has none; the `networksetup` and
  GNOME equivalents are not wired up yet, so loopback and intranet traffic on
  those platforms follows whatever the desktop was already configured with

Already shipped, and previously listed here in error: macOS support (via
`networksetup`), Linux support (GNOME/KDE and environment variables), and PAC
file generation — see the sections above and `GET /api/v1/pac`.
