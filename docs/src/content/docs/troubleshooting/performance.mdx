...
:::caution[Inbound connections are not reused]
The HTTP listener handles exactly one request per accepted connection and then
closes it, so raising `idle_timeout` will **not** make clients reuse inbound
connections — earlier revisions of this page said it would. `idle_timeout`
bounds a connection with nothing in flight (see
[Listener timeouts](/configuration#listener-timeouts)); it does not create a
keep-alive loop. `CONNECT` tunnels, which carry all HTTPS traffic, are
unaffected either way: one tunnel serves the whole TLS session.
:::

What genuinely reduces per-request setup cost:

- **Route HTTPS through `CONNECT`** (the default for any `https://` origin), so
  one tunnel carries many requests instead of one connection per request.
- **Tune outbound keep-alive** in the top-level `network` section, so the
  backend-side connection is not re-established for every request:

```yaml
network:
  keepalive: "30s"  # TCP keep-alive period for outbound dials
  dial_timeout: "10s"
```

- **Enable caching** so repeat requests never reach a backend at all.

:::note[Outbound pooling is not configurable]
Bifrost does not expose connection-pool knobs (idle connection counts, per-host
limits, idle connection lifetime) for outbound requests — Go's HTTP transport
defaults apply. Tune `network.keepalive` instead.
:::
...
1. Use TLS session resumption (automatic in Go)
2. Route HTTP/2-capable traffic through `CONNECT` tunnels: ALPN is
   end-to-end there, so h2 works through the tunnel even though the proxy's
   own plain-HTTP path speaks HTTP/1.1 with one request per connection
...
Limit cache memory usage. All memory and disk settings live under
`cache.storage`; use `tiered` storage to keep small objects in RAM and spill
larger ones to disk:
...
cache:
  ...
  storage:
    ...
    disk:
      ...
      max_size: "1GB"
    tiered:
      memory_threshold: "1MB"  # Larger objects go to disk
```

:::caution[`storage.type` decides what is required]
`type: memory` requires a `memory` block, `type: disk` requires a `disk` block
with a `path`, and `type: tiered` requires both. Any other value is rejected at
startup.
:::

### Cause 3: Connection Growth

Too many simultaneous connections inflate memory usage.

**Solution:**

Cap concurrent connections per listener and process-wide:

```yaml
server:
  http:
    max_connections: 10000  # Per listener, 0 = unlimited

network:
  max_connections: 20000  # Across all listeners, 0 = unlimited
```

:::note[No pool tuning]
Idle connection pooling for outbound requests is not configurable — Go's HTTP
transport defaults apply. Only the connection ceilings above and the listener
...
For a WireGuard backend (server config):
...
For client VPN mode, the MTU belongs to the TUN device:
...
server:
  http:
    ...
    read_timeout: "30s"
    write_timeout: "30s"
    idle_timeout: "120s"   # also reaps quiet CONNECT tunnels; raise for SSH/IMAP IDLE
