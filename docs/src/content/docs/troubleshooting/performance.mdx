...
Keep inbound connections alive longer so clients can reuse them:

```yaml
server:
  http:
    idle_timeout: "120s"  # Keep client connections alive longer
```

Outbound keep-alive is tuned in the top-level `network` section:
...
defaults apply. Tune `idle_timeout` on the listener and `network.keepalive`
instead.
:::
