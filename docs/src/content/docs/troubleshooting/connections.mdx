...
Correct — one specific interface:
...
:::caution[One `server:` key per file]
These are three alternatives, not one config. Repeating a top-level key such as
`server:` in the same YAML file is a duplicate mapping key and the server
refuses to start.
:::
...
Increase the listener timeouts. Each listener supports `read_timeout`,
`write_timeout` and `idle_timeout`:
...
Pick the one that matches the symptom — see
[Listener timeouts](/configuration#listener-timeouts) for the exact meaning of
each:

| Symptom | Setting |
|---------|---------|
| A slow client's request is cut off before it finishes arriving | `read_timeout` |
| A response to a client that stopped reading is cut off | `write_timeout` |
| An accepted-but-silent connection is closed too eagerly | `idle_timeout` |
| A `CONNECT` tunnel or SOCKS5 relay dies while sitting quiet | `tunnel_idle_timeout` — off by default, so if a quiet tunnel dies, something else closed it |

Outbound dial timeouts are a property of the backend, not the listener, and are
set in the backend's `config` block:
...
A process-wide default for backends that do not set their own is
...
:::note[Raising `read_timeout` no longer affects backend connects]
Up to and including v1.0.0 the HTTP listener's `read_timeout` was passed through
as the outbound dial timeout, and neither `write_timeout` nor `idle_timeout` was
applied at all. It is now the inbound read deadline it always claimed to be. If
you had raised `read_timeout` to work around slow backend connects, move that
value to `network.dial_timeout` (or the backend's own `connect_timeout`) —
otherwise the effective dial timeout drops back to the 30s default.
:::
...
### Cause 2: SOCKS5 Listener Not Configured

The SOCKS5 listener may not be configured. There is no `enabled` flag on a
listener — a listener is active when its `listen` address is set, and omitting
`listen` (or leaving it empty) disables it.
...
Give the SOCKS5 listener an address:
