...
# Requests by status (compute error rate from 5xx statuses)
curl -s http://localhost:7090/metrics | grep bifrost_requests_total

# Backend health
curl -s http://localhost:7090/metrics | grep bifrost_backend_health

# Backend latency
curl -s http://localhost:7090/metrics | grep bifrost_backend_latency_seconds

# Memory usage (from the process collector)
curl -s http://localhost:7090/metrics | grep process_resident_memory_bytes

# Goroutine count
curl -s http://localhost:7090/metrics | grep bifrost_goroutines

# Cache statistics (via the REST API — cache metrics are not exported to Prometheus)
curl -s http://localhost:7082/api/v1/cache/stats
```
...
# Error rate percentage (share of requests with a 5xx status)
sum(rate(bifrost_requests_total{status=~"5.."}[5m])) / sum(rate(bifrost_requests_total[5m])) * 100
...
# P99 backend health-check latency
histogram_quantile(0.99, rate(bifrost_backend_latency_seconds_bucket[5m]))
```
