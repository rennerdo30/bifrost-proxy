...
Backend endpoints live on the server API (default `:7082`); VPN and mesh endpoints live on the
client API (default `127.0.0.1:7383`).

```bash
# Check backend health (server API)
curl -s http://localhost:7082/api/v1/backends | jq '.[] | {name, type, healthy}'

# Check VPN status (client API)
curl -s http://localhost:7383/api/v1/vpn/status | jq
...
vpn:
  ...
  tun:
    name: bifrost0
    # Address is assigned to the interface on startup
...
```bash
# Linux
sudo ip addr add 10.255.0.1/24 dev bifrost0
...
1. Check split tunnel configuration. `mode` is the only switch, and it takes one of two values:

   ```yaml
   # All traffic goes through the VPN except the destinations listed below
   vpn:
     split_tunnel:
       mode: exclude
   ```
   ...
   ```yaml
   # Only the destinations listed below go through the VPN
   vpn:
     split_tunnel:
       mode: include
   ```
...
curl -s http://localhost:7383/api/v1/vpn/split/rules | jq
```
...
vpn:
  split_tunnel:
    ...
    # App-based (process name, optional full path)
    apps:
      - name: "Slack"
      - name: "Zoom"

    # Checked before everything else, in either mode
    always_bypass:
      - "127.0.0.0/8"
```
...
Use a reliable health check target. For `type: http` the probe URL is built as
`<scheme>://<target><path>`, so put the host in `target` and keep the scheme and path in their own
fields — a full URL in `target` does not work:
...
health_check:
  ...
  path: "/generate_204"   # Google's connectivity check
```

For `type: tcp` the target is a plain `host:port`:
...
```bash
# Check mesh node status (peer ID, virtual IP, direct vs relayed connections)
curl -s http://localhost:7383/api/v1/mesh/status | jq

# Check discovered peers
curl -s http://localhost:7383/api/v1/mesh/peers | jq

# Check which peers are actually connected
curl -s http://localhost:7383/api/v1/mesh/peers/connected | jq
```

**Solution:**

1. **Symmetric NAT detected**: Enable TURN relay. STUN and TURN are subsections of `mesh`, not
   top-level keys:
...
# Mesh status (includes direct vs relayed connection counts)
curl -s http://localhost:7383/api/v1/mesh/status | jq
