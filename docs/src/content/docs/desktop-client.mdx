...
description: "Documentation for Desktop Client"
---
...
The log level lives in the top-level `logging` section, not under `debug`. The `debug` section
controls traffic capture only: `enabled`, `max_entries`, `capture_body`, `max_body_size` and
`filter_domains`.
