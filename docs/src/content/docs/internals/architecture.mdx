...
graph TB
    ...
    subgraph "Shared State"
        BackendMgr[Backend Manager]
        Router[Router]
        ActiveConns[Active Connection Count]
    end
    ...
    WG -.->|Track| HTTPServe
    WG -.->|Track| SOCKSServe
    Mutex -.->|Protect| BackendMgr
    Mutex -.->|Protect| Router
    Atomic -.->|Count| ActiveConns
...
metrics:
  collection_interval: "60s"    # Reduce from the 15s default

api:
  websocket_max_clients: 10     # Reduce from the 100 default

server:
  http:
    max_connections: 1000       # Per-listener ceiling (0 = unlimited)

network:
  max_connections: 1000         # Process-wide ceiling across all listeners
```

The connection ceilings live on the listener (`server.http.max_connections`,
`server.socks5.max_connections`) and process-wide under `network`; there is no
`connection_limits` or top-level `websocket` section.
