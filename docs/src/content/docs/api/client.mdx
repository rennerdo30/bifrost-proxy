...
| Field | Meaning |
|-------|---------|
| `bytes_sent` | Cumulative bytes written back to local proxy clients since the client started |
| `bytes_received` | Cumulative bytes read from local proxy clients since the client started |
| `active_connections` | Proxy connections currently being served across the HTTP and SOCKS5 listeners |

The byte counters cover both the HTTP and SOCKS5 listeners and are reset when
the client process restarts (they are in-memory, not persisted).
...
Field names match the YAML configuration keys exactly, so a response can be
edited and sent straight back to `PUT /api/v1/config`.

Durations are duration strings, not numbers — `"30s"`, `"1m30s"`, `"5m0s"` — in
both directions. This covers `vpn.dns.cache_ttl`,
`mesh.discovery.heartbeat_interval`, `mesh.discovery.peer_timeout`,
`mesh.stun.timeout`, `mesh.connection.connect_timeout` and
`mesh.connection.keep_alive_interval`, alongside the `proxy` and `server`
timeouts. On input a bare number is also accepted and read as nanoseconds.
...
```json
{
  ...
  "action": "server",
  "matched_route": "corp-apis"
}
```

`matched_route` names the rule that produced the decision. It is **omitted**
when no rule matched and the default action applied, so its absence is
meaningful: the domain fell through to the default rather than matching a rule
