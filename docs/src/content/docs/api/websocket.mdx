...
An origin check compares the browser's `Origin` against the request's `Host`, so
it does not defend against **DNS rebinding**: an attacker who controls a
hostname can point it at your Bifrost's address, at which point their page's
`Origin` and the request's `Host` legitimately agree. Where that matters, put
Bifrost behind authentication (`api.token`) and a proxy that rejects unexpected
`Host` headers, rather than relying on the origin check alone.
...
This is an escape hatch for setups whose origin cannot be predicted. It logs a
warning at startup, and it means any web page opened in a browser that can reach
this server may connect to `/api/v1/ws` and read the live traffic stream. Name
the real origins instead wherever you can.
