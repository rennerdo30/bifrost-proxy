...
> A bare `$VAR` is **not** expanded, and every other `$` is literal, so a
> password or bcrypt hash containing `$` is kept verbatim. A reference to a
> variable that is not set expands to the empty string and logs a warning with
> the line number — use `${VAR:-fallback}` where that is not acceptable. See
