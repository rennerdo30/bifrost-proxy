...
> Expansion uses Go's `os.ExpandEnv`: only `${VAR}` and `$VAR` are recognised.
> Shell-style defaults like `${BIFROST_LOG_LEVEL:-info}` are **not** supported —
> they expand to the empty string. Export every variable the config references.
