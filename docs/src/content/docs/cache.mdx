...
Cache statistics are **not** exported as Prometheus metrics. Instead, retrieve
them from the REST API (also surfaced in the Web UI):

```bash
curl http://localhost:7082/api/v1/cache/stats
```

This returns hit/miss counts, bytes served, entry counts, and per-tier storage
usage. See the [API reference](/api) for the full response schema.
