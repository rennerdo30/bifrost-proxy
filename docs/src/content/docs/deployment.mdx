...
description: "Documentation for Deployment Guide"
---
...
Bifrost has **built-in size-based rotation** for file outputs. Set `max_size_mb`
and `max_backups` in the `logging` section:
...
logging:
  ...
  max_size_mb: 100    # Rotate when the file exceeds 100 MB (<= 0 disables rotation)
  max_backups: 14     # Keep 14 rotated files (<= 0 keeps all)
```

Rotated files are renamed with a `YYYYMMDD-HHMMSS.mmm` timestamp suffix (e.g.
`server.log.20260702-103000.123`). The suffix sorts chronologically, which is how
`max_backups` decides which files to prune.

Alternatively, if you prefer external rotation, disable the built-in rotation
(leave `max_size_mb` unset or `0`) and use `logrotate` with `copytruncate` (the
process keeps the same file handle, so a plain rename would not be picked up).
...
> System authentication (an `auth.providers[]` entry with `type: system`) is **not supported on Windows**.
