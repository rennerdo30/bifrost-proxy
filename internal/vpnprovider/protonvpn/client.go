...
func (c *Client) GenerateOpenVPNConfig(ctx context.Context, server *vpnprovider.Server, creds vpnprovider.Credentials) (*vpnprovider.OpenVPNConfig, error) {
	...
	// Generate config content. The CA certificate is required and supplied via
	// configuration; generation fails closed if it is missing or invalid.
	config, err := c.generateOpenVPNConfigContent(server, "udp", creds)
	if err != nil {
		return nil, err
	}
...
func (c *Client) generateOpenVPNConfigContent(server *vpnprovider.Server, protocol string, creds vpnprovider.Credentials) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("%w: ProtonVPN CA certificate could not be re-encoded: %w",
			vpnprovider.ErrConfigGenerationFailed, err)
	}
