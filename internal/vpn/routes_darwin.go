...
func (r *darwinRouteManager) Setup(ctx context.Context, tunName string, cfg Config) error {
	...
	if cfg.SplitTunnel.Mode == "exclude" {
		...
		// that override the default route without replacing it directly. These
		// ARE the tunnel — without either of them traffic leaks over the
		// physical interface.
...
func (r *darwinRouteManager) rollbackLocked() {
...
// already holds r.mu.
func (r *darwinRouteManager) addBypassRouteLocked(destination string) error {
	// Validate CIDR
