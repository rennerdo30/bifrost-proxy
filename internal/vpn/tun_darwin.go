...
func createPlatformTUN(cfg TUNConfig) (TUNDevice, error) {
	...
	if err != nil {
		...
		return nil, &TUNError{Op: "socket", Err: err}
	}
	...
	if err := unix.IoctlCtlInfo(fd, ctlInfo); err != nil {
		unix.Close(fd)
		return nil, &TUNError{Op: "ioctl CTLIOCGINFO", Err: err}
	}
	...
	if err := unix.Connect(fd, sa); err != nil {
		...
		if err == syscall.EBUSY {
			return nil, ErrTUNAlreadyExists
		}
		return nil, &TUNError{Op: "connect", Err: err}
	}
	...
	if err := unix.SetNonblock(fd, true); err != nil {
		unix.Close(fd)
		return nil, &TUNError{Op: "set nonblock", Err: err}
	}
...
func getUtunName(fd int) (string, error) {
	...
	if errno != 0 {
		return "", &TUNError{Op: "getsockopt UTUN_OPT_IFNAME", Err: errno}
	}
...
func (t *darwinTUN) configure(cfg TUNConfig) error {
	...
	if addr.Is4() {
		...
		if output, err := cmd.CombinedOutput(); err != nil {
			return &TUNError{Op: "ifconfig address", Err: fmt.Errorf("%w: %s", err, string(output))}
		}
	} else {
		// IPv6
		cmd := exec.Command("ifconfig", t.name, "inet6", fmt.Sprintf("%s/%d", addr, bits))
		if output, err := cmd.CombinedOutput(); err != nil {
			return &TUNError{Op: "ifconfig address6", Err: fmt.Errorf("%w: %s", err, string(output))}
		}
	}
	...
	if output, err := cmd.CombinedOutput(); err != nil {
		return &TUNError{Op: "ifconfig mtu", Err: fmt.Errorf("%w: %s", err, string(output))}
	}

	// Bring interface up
	cmd = exec.Command("ifconfig", t.name, "up")
	if output, err := cmd.CombinedOutput(); err != nil {
		return &TUNError{Op: "ifconfig up", Err: fmt.Errorf("%w: %s", err, string(output))}
	}
	...
	for {
		...
		if err != nil {
			if err == syscall.EAGAIN || err == syscall.EWOULDBLOCK {
				...
				if err != nil {
					return 0, &TUNError{Op: "select", Err: err}
				}
				continue
			}
...
// This can be useful for integration with other code.
